...
describe('buildICS — yapı', () => {
  ...
  it('tüm gün etkinliğinde DTEND ertesi gündür', () => {
    ...
    const s = satirlar(buildICS([etkinlik({ date: '2026-06-15' })]))
  ...
  it('UID her dışa aktarımda aynı kalır', () => {
    ...
    const a = satirlar(buildICS([etkinlik()])).find(l => l.startsWith('UID:'))
    const b = satirlar(buildICS([etkinlik()])).find(l => l.startsWith('UID:'))
    expect(a).toBe(b)
...
describe('buildICS — kaçış (RFC 5545 kuralları CSV kurallarından farklı)', () => {
  it('virgül, noktalı virgül ve ters bölüyü kaçırır', () => {
    const s = buildICS([etkinlik({ label: 'Yag; filtre, conta \\ ek' })])
    const ozet = satirlar(s).find(l => l.startsWith('SUMMARY:'))
    expect(ozet).toContain('Yag\\; filtre\\, conta \\\\ ek')
  })

  it('tırnağı KAÇIRMAZ (CSV kuralı burada geçerli değil)', () => {
    const s = buildICS([etkinlik({ label: 'Servis "Usta"' })])
    expect(satirlar(s).find(l => l.startsWith('SUMMARY:'))).toContain('Servis "Usta"')
  })

  it('satır sonunu kaçış dizisine çevirir, gerçek satır sonu bırakmaz', () => {
    const s = buildICS([etkinlik({ label: 'Bir\nIki' })])
    const ozetSatiri = satirlar(s).find(l => l.startsWith('SUMMARY:'))
    expect(ozetSatiri).toContain('Bir\\nIki')
  })
})

describe('buildICS — 75 oktet satır katlama', () => {
  it('uzun satırı katlar ve devam satırları boşlukla başlar', () => {
    const s = buildICS([etkinlik({ label: 'A'.repeat(200) })])
    const tumSatirlar = satirlar(s)
    const bas = tumSatirlar.findIndex(l => l.startsWith('SUMMARY:'))
    expect(tumSatirlar[bas + 1].startsWith(' ')).toBe(true)
  })

  it('hiçbir satır 75 okteti aşmaz', () => {
    const s = buildICS([etkinlik({ label: 'Şşğüöç'.repeat(40) })])
    const enUzun = Math.max(...satirlar(s).map(l => new TextEncoder().encode(l).length))
  ...
  it('çok baytlı karakteri ortasından bölmez', () => {
    ...
    const metin = 'Ğ'.repeat(120)
    const tum = satirlar(buildICS([etkinlik({ label: metin })]))
    ...
    expect(birlesik).toContain(metin)
...
describe('buildICS — hatırlatma', () => {
  it('yasal tarihlere alarm ekler', () => {
    const s = satirlar(buildICS([etkinlik({ type: 'mtv', label: 'MTV' })], { hatirlatmaGun: 7 }))
  ...
  it('geçmiş bakım kaydına alarm eklemez', () => {
    ...
    const s = satirlar(buildICS([etkinlik({ type: 'maintenance', label: 'Yağ Değişimi' })]))
    expect(s).not.toContain('BEGIN:VALARM')
  })

  it('hatirlatmaGun 0 ise hiç alarm eklenmez', () => {
    const s = satirlar(buildICS([etkinlik()], { hatirlatmaGun: 0 }))
    expect(s).not.toContain('BEGIN:VALARM')
  })
})
...
describe('buildICS — dayanıklılık', () => {
  ...
  it('tarihsiz veya araçsız etkinliği atlar', () => {
    const s = buildICS([
      etkinlik(),
      { ...etkinlik(), date: '' },
      { ...etkinlik(), vehicle: null as unknown as Vehicle },
    ])
    expect(satirlar(s).filter(l => l === 'BEGIN:VEVENT')).toHaveLength(1)
  })
})
...
describe('buildVehicleEvents -> buildICS bütünü', () => {
  ...
  it('aracın dört yasal tarihini de takvime taşır', () => {
    ...
    const etkinlikler = buildVehicleEvents([arac])
    expect(etkinlikler).toHaveLength(4)
    // type makine anahtarı, label insan metni — eski Dashboard şeklinde ikisi tek alandaydı
    expect(etkinlikler.map(e => e.type)).toEqual(['inspection', 'mtv', 'insurance', 'kasko'])
    expect(etkinlikler.map(e => e.label)).toEqual(['Muayene', 'MTV', 'Sigorta', 'Kasko'])

    const s = satirlar(buildICS(etkinlikler))
  ...
  it('tarihi olmayan alanları atlar', () => {
    const arac = { ...ARAC, inspectionDate: '2026-06-15', mtvDate: '' } as unknown as Vehicle
    expect(buildVehicleEvents([arac])).toHaveLength(1)
  })

  it('bakım ve yakıt kayıtlarını da isteğe bağlı ekler', () => {
    const arac = { ...ARAC, inspectionDate: '2026-06-15' } as unknown as Vehicle
    const etkinlikler = buildVehicleEvents([arac], {
      bakimlar: [{ id: 'm1', vehicleId: 'v1', date: '2026-01-10', type: 'Yağ Değişimi' }] as never,
      yakitlar: [{ id: 'f1', vehicleId: 'v1', date: '2026-02-10', liters: 45, totalCost: 2000 }] as never,
    })
    expect(etkinlikler.map(e => e.type)).toEqual(['inspection', 'maintenance', 'fuel'])
    expect(etkinlikler[2].label).toContain('45 L')
  })
  ...
  it('bilinmeyen araca ait kaydı atlar', () => {
    ...
    expect(etkinlikler).toHaveLength(0)
  })
})
